...
export const MODULE_LECCIONES: Record<string, LeccionContenido[]> = {
  ...
  "navegacion-2": [
    ...
    {
      titulo: "Técnicas avanzadas: arco DME y patrón de espera",
      temas: [
      { id: "navegacion-tema25", titulo: "Arco DME", texto: "Un arco DME es una trayectoria curva que vuelas manteniendo una distancia constante respecto a una estación — por ejemplo, el arco de 12 millas náuticas alrededor de un VOR/DME. Sirve para conectar una aerovía o un radial con el tramo inicial de una aproximación sin tener que sobrevolar la estación, y es de las pocas maniobras de radionavegación en las que no vuelas en línea recta. La técnica estándar consiste en volar perpendicular a la estación (90° respecto al radial en el que te encuentras) y corregir en pequeños incrementos: dejas que la distancia se desvíe ligeramente, giras unos 10° hacia dentro o hacia fuera del arco según corresponda, y mueves el OBS otros 10° para el siguiente segmento — de ahí el nombre coloquial de \"girar y torcer\" (turn and twist). El viraje para entrar al arco se anticipa un poco antes de alcanzar la distancia objetivo; como referencia se usa alrededor de media milla náutica a velocidades de aviación general, valor que conviene ajustar según tu velocidad respecto al suelo." },
      { id: "navegacion-tema26", titulo: "Patrón de espera", texto: "Un patrón de espera (holding) es un circuito en forma de hipódromo que vuelas alrededor de un fix para retrasar tu llegada de forma ordenada, ya sea porque control te lo asigna por congestión de tráfico o porque forma parte de un procedimiento publicado. Se define por cuatro datos: el fix, el curso de entrada (inbound), el lado de los virajes (a la derecha por convención estándar, salvo que se publique lo contrario) y la duración del tramo. Como referencia ampliamente enseñada, los tramos son de un minuto a 14,000 pies o menos y de minuto y medio por encima, volados con virajes de tasa estándar (3° por segundo) — confirma los valores exactos en la publicación vigente. La entrada que te corresponde se determina por el ángulo entre tu rumbo de llegada y el curso de entrada, que divide el espacio alrededor del fix en tres sectores: directa, paralela y en gota (teardrop). El viento se corrige en el tramo de alejamiento, donde se aplica aproximadamente el triple del ángulo de corrección usado en el tramo de acercamiento, ajustando además el tiempo de alejamiento para que el de acercamiento dure lo previsto." },
      ],
    },
  ],
  "cartografia": [
